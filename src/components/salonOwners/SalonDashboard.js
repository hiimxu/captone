...
export default function Schedule() {
  ...
  const [dateHistory, setDateHistory] = useState(new Date().toISOString().substring(0, 10));
  ...
  useEffect(() => {
    dispatch(getSalonBookingHistory(token, { day: dateHistory }));
    return () => {
      dispatch(resetSalonBookingHistoryList());
    };
  }, [dispatch, token, dateHistory]);
                  ...
                  <div className="has-text-centered">
                    <h1 className="is-size-4 ">
                      ...
                      <p className="is-size-3">
                        ...
                        {orderIdCancel.nameService} - {orderIdCancel.nameStaff}{" "}
                        -{" "}
                        {convertISOStringToLocaleTimeString(
